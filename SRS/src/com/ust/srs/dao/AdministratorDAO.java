...
public class AdministratorDAO implements Administrator {
    ...
    public int removeRoute(ArrayList<String> routeIds) {
        int removedCount = 0;
        try {
            // SQL query to delete from route table based on routeID
            String query = "DELETE FROM route WHERE routeID = ?";
            PreparedStatement ps = DB.getCon().prepareStatement(query);

            // Loop through all route IDs and execute the deletion
            for (String routeId : routeIds) {
                ps.setString(1, routeId);
                removedCount += ps.executeUpdate(); // Increment count for each successful deletion
            }
        } catch (SQLException e) {
            e.printStackTrace(); // Print any SQL errors
        }
        return removedCount; // Return the total number of rows deleted
    }
    ...
    public RouteBean viewByRouteId(String routeID) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                route.setTravelDuration(rs.getString("travelDuration"));
    ...
    public ArrayList<ShipBean> viewByAllShips() {
        ArrayList<ShipBean> ships = new ArrayList<>();
        try {
    ...
    public ArrayList<ScheduleBean> viewByAllSchedule() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                schedule.setShipID(rs.getString("shipID"));
    ...
    public ArrayList<PassengerBean> viewPassengersByShip(String scheduleID) {
        ...
        try {
            String query = "SELECT * FROM passenger WHERE scheduleID = ?";
            PreparedStatement ps = DB.getCon().prepareStatement(query);
            ps.setString(1, scheduleID);
            ResultSet rs = ps.executeQuery();
            ...
            while (rs.next()) {
                ...
                passenger.setScheduleID(rs.getString("scheduleID"));
